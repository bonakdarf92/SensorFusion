...
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud, float filterRes, Eigen::Vector4f minPoint, Eigen::Vector4f maxPoint)
{
    ...
    voxels.setLeafSize(filterRes, filterRes, filterRes);
    voxels.filter(*cloudFilter);
...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SegmentPlane(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceThreshold)
{
    ...
    pcl::SACSegmentation<PointT> segs;
    segs.setOptimizeCoefficients(true);
    segs.setModelType(pcl::SACMODEL_PLANE);
    segs.setMethodType(pcl::SAC_RANSAC);
    segs.setMaxIterations(maxIterations);
    segs.setDistanceThreshold(distanceThreshold);

    pcl::ExtractIndices<pcl::PointXYZ> extract;
    segs.setInputCloud(cloud);
    segs.segment(*inliers, *coefficients);
    if (inliers->indices.size() == 0) {
        std::cout << "Konnte keine planare Ebene in Punktwolke finden" << std::endl;
    }


    auto endTime = std::chrono::steady_clock::now();
    auto elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
    std::cout << "Plane Segmentierung dauert" << elapsedTime.count() << " milliseconds" << std::endl;
